...
AI2-THOR 的真实实验数据、第一视角视频和实体机器人结果在完成对应实验前均标记为“待验证”，不会用二维数据替代。

## 联系与引用

本项目适合作为保研面试中的科研项目展示：重点关注 Agent-Tool 架构、可验证实验设计和从二维受控环境向三维仿真的迁移路线。引用或复现实验时，请同时保留实验配置、`summary.json` 和逐回合指标文件。
